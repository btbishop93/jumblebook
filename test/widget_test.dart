...
void main() {
  ...
  setUpAll(() {
    // Auth Events
    registerFallbackValue(SignInWithEmailRequested(email: 'test@email.com', password: 'password'));
    registerFallbackValue(SignUpWithEmailRequested(email: 'test@email.com', password: 'password'));
  ...
  setUp(() {
    ...
    when(() => mockAuthBloc.add(any(that: isA<AuthEvent>()))).thenAnswer((_) async {});
    ...
    when(() => mockThemeBloc.add(any(that: isA<ThemeEvent>()))).thenAnswer((_) async {});
